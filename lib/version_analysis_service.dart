...
class VersionAnalysisService {
  ...
  Future<void> analyzeVersionConstraints(
    ...
    for (final dependent in dependents) {
      ...
      if (constraint == null) {
        print(
          'Skipping ${dependent.packageName} - invalid constraint: $constraintString',
        );
        continue;
      }
  ...
  Future<void> generateVersionCompatibilityReport(
    ...
    final sortedMinorVersions =
        versionsByMinor.keys.toList()..sort((a, b) {
    ...
    final targetMinorVersions =
        sortedMinorVersions.take(lastNMinorVersions).toList();

    print(
      'Analyzing ${targetMinorVersions.length} minor versions: ${targetMinorVersions.join(', ')}',
    );
    ...
    for (final minorVersion in targetMinorVersions) {
      ...
      final supportedDependents = await _packageDataService
          .getDependentsSupportingVersion(
            targetPackage,
      ...
      final supportPercentage =
          totalCount > 0 ? (supportedCount / totalCount) * 100 : 0.0;
      ...
      print(
        '${latestPatchVersion.version}: $supportedCount/$totalCount dependents (${supportPercentage.toStringAsFixed(1)}%)',
      );
    }
...
extension VersionAnalysisExtension on PackageDataService {
  ...
  Future<List<String>> getDependentsSupportingVersion(
    String targetPackage,
    String version,
  ) async {
    final database = PackageDatabase();
    final results = await database.getDependentsSupportingVersion(
      targetPackage,
      version,
    );
    await database.close();
  ...
  /// Retrieves dependents that support a specific version with their constraint strings
